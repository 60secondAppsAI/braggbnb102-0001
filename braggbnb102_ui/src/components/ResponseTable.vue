        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalResponses = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalResponses">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Response</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="ResponseId" type="text" placeholder="Enter ResponseId" v-model="responseToAdd.responseId"></base-input>
  <base-input label="ResponseText" type="text" placeholder="Enter ResponseText" v-model="responseToAdd.responseText"></base-input>
  <base-input label="ResponseDate" type="text" placeholder="Enter ResponseDate" v-model="responseToAdd.responseDate"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="responses" :row-key="record => record.ResponseId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <ResponsePictureView :responses="responses" />
          </div>
...
import ResponsePictureView from './ResponsePictureView.vue';
...
export default {
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'responseId',
          visible: true,
          scopedSlots: { customRender: 'responseId' },
          sorter: true
          //sorter: (a, b) => a.responseId - b.responseId,
          //sorter: (a, b) => a.responseId.localeCompare(b.responseId),
        },
        {
          title: 'Response Text',
		dataIndex: 'responseText',
          ...
          sorter: true
          //sorter: (a, b) => a.responseText - b.responseText,
          //sorter: (a, b) => a.responseText.localeCompare(b.responseText),
        },
        {
          title: 'Response Date',
      ...
      sortBy: 'responseId',           // Column to sort by
  ...
  methods: {
    ...
    async renderResponsesTable() {
      ...
      let responsesTableData = [];
      for (let i = 0; i < this.responses.length; i++) {
        responsesTableData.push({
          id: i,
          responseId: this.responses[i].responseId,
          year: this.responses[i].year,
          date: this.responses[i].date,
          competitionId: this.responses[i].competitionId,
          responseId: this.responses[i].responseId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-responses',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForResponsesChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalResponses = false;

      const currentDate = new Date().getTime();
      this.responseToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.responseToAdd);
      console.log(jsonData);
      
      const res = await ResponseService.addResponse(jsonData);
