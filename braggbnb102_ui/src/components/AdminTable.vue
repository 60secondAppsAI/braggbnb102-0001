        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalAdmins = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalAdmins">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Admin</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="AdminId" type="text" placeholder="Enter AdminId" v-model="adminToAdd.adminId"></base-input>
  <base-input label="Level" type="text" placeholder="Enter Level" v-model="adminToAdd.level"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="admins" :row-key="record => record.AdminId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <AdminPictureView :admins="admins" />
          </div>
...
import AdminPictureView from './AdminPictureView.vue';
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'adminId',
          ...
          //sorter: (a, b) => a.adminId.localeCompare(b.adminId),
        },
        {
          title: 'Level',
		dataIndex: 'level',
      ...
      sortBy: 'adminId',           // Column to sort by
  ...
  methods: {
    ...
    async renderAdminsTable() {
      ...
      for (let i = 0; i < this.admins.length; i++) {
        adminsTableData.push({
          id: i,
          adminId: this.admins[i].adminId,
          year: this.admins[i].year,
          date: this.admins[i].date,
          competitionId: this.admins[i].competitionId,
          adminId: this.admins[i].adminId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-admins',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForAdminsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalAdmins = false;

      const currentDate = new Date().getTime();
      this.adminToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.adminToAdd);
      console.log(jsonData);
      
      const res = await AdminService.addAdmin(jsonData);
